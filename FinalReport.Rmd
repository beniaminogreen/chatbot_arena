...
```{r, figure.width=10, fig.height=10, eval = F, include = F}
## TODO: big_grid is not created in this notebook - we need to create it again so that we can render the final results
...
Because the simple model can't generate predictions for all outcomes, e try to
...
model. The fourth panel does not have a best-fit line, becasue this model
predicts the same probability of 'both-bad' for each dyad hence there is no
...
![Leaderboard Produced by Flagship Model 2. This is substantially identical to the leaderboard produced by the Bradley-Terry model](leaderboard_plots/derail_model.png)
