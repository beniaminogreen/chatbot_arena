...
We test this model using a series of calibration plots and by evaluating its
cross-entropy loss. High-cross entropy loss suggests that the model does
assigns very low probability to the observed outcome of each match, while a
low-cross entropy loss suggests that the model assigns high probability to
observed outcomes

```{r, fig.width = 8, fig.height = 8, message = F, echo = F, warning = F}
...
output <- full_join(matchups_full,x, 
  by = c("player1" = "model_a","player2" = "model_b")
) %>%
    ...
    summarize(
      perc_a_win = sum(winner == "model_a") / n(),
      perc_b_win = sum(winner == "model_b") / n(),
      perc_tie = sum(winner == "tie") / n(),
      perc_bb = sum(winner == "both_bad") / n(),
...
create_calibration_plots(output)
```

The model appears to give reasonably calibrated estimates for the 'tie' and
'both-bad' outcomes, but there is a clear attenuation when it comes to
predicting the outcomes with a winner. Specifically, the model under-estimates
the probabilities of winning / loosing in extreme matchups when two models with very different skill levels play each other, resulting in predictions that understate the imbalance in the matchup.
...
predicting the outcome of each match. 
...
Given that this model is the best-performing out of the ones we consider, we
report predictions from these models in the `prediction.csv` submitted with our
assignment.
...
![Leaderboard Produced by Flagship](leaderboard_plots/derail_model.png)
...
each of the models has an independent chance of derailing the matchup and
forcing a 'both-bad' outcome. We report predictions and leaderboard rankings
from this model, but our estimates of model performance are substantially
